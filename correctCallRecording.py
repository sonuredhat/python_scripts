...
def findFiles(agencyName):
    baseDir = "/blobContainer/recordings/2025/04"
    #day = datetime.now().day  # Get the current day as an integer
    day = 25
    ...
    for dayNum in range(18, day):  # Loop from day 10 up to (but not including) today
        dayPath = f"{baseDir}/{dayNum:02d}/{agencyName}/out/"  # format day as 2-digit
        if not os.path.exists(dayPath):
            # print(f"Path does not exist: {dayPath}")
            logger.log_warning(f"Path does not exist: {dayPath}")   
            continue
        
        print(f"Searching in: {dayPath}")
        for root, _, files in os.walk(dayPath):
            for file in files:
                file_path = os.path.join(root, file)
                temp = (file_path.split("_")[-1])
                fileCorrectDate = temp.split(".")[0][6:8]
                correctPath = f"{baseDir}/{fileCorrectDate}/{agencyName}/out/{file}"
                # print(f"correctPath: {correctPath}")
                # print(f"file current path: {file_path}")
                logger.log_info(f"correctPath: {correctPath}")
                logger.log_info(f"file current path: {file_path}")  
                if correctPath != file_path:
                    # print(f"File needs to be moved from {file_path} to {correctPath}")
                    logger.log_info(f"File needs to be moved from {file_path} to {correctPath}")
                    shutil.move(file_path, correctPath)
                else:
                    print(f"File is already in the correct path: {file_path}")
                    logger.log_info(f"File is already in the correct path: {file_path}")
                    
if __name__ == "__main__":
    agency="Bhumi_2_5"
    findFiles(agency)
    logger.log_info(f"Correction Completed for: {agency}")
